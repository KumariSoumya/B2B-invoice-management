...
const Functionalities = () => {
  ...
  const delData = async () => {
    for (let i in selectedRows) {
      await axios.get(
        `http://localhost:8080/backendHRC/DeleteServlet?sl_no=${selectedRows[i]}`
      );
    }
  };
  const AdvData = async () => {
    let res = await axios.get(
      `http://localhost:8080/backendHRC/SearchServlet?doc_id=${AdvVal.doc_id}&invoice_id=${AdvVal.invoice_id}&cust_number=${AdvVal.cust_number}&buisness_year=${AdvVal.buisness_year}`
    );
